...
def loadConfig():
	...
	if (len(sys.argv) > 2):
		config['gameN'] = int(sys.argv[2])

	if (len(sys.argv) > 3):
		config['max_clients'] = sys.argv[3]

	if ('gameN' in config):
		config['gameN'] = int(config['gameN'])

	# TODO: Some keys are REQUIRED. maybe check that they are present and also 
	#       that they have valid values.
	pr_okblue('server config: ')
	for (k, v) in config.iteritems():
		pr_okblue ('  ' + str(k) + ': ' + str(v))
	print 
	...
	while True:
		...
		if (cmd.startswith('q')):
			...
			try:
				...
				Error('Bad Input')
				continue
			game.startWar(players[0], players[1], config['gameN'])
