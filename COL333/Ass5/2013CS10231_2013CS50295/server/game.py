...
def printBoardX(b):
	...
	col_header = (
		'   ' + 
		' '.join([str(x) for x in range(gameN)]) + 
		' (cols)'
	)
	pr_okred(col_header)
...
def isValidMoveByOrder(a, b, c, d):
	...
	if (a == c):
		colrange = range(b+1, d+1) if b < d else range(b-1, d-1, -1)
		for col in colrange:
			if (board[a][col] != '-'):
				return False

	if (b == d):
		rowrange = range(a+1, c+1) if a < c else range(a-1, c-1, -1)
		for row in rowrange:
			if (board[row][b] != '-'):
				return False
...
def calculateScore():
	if (timers[0] < 0):
		...
		return MAX_SCORE[gameN]	# Some REASONABLY good score should be given!
...
def startWar(bot1, bot2, board_sz=5):
	global board, gameN
	gameN = board_sz if board_sz >= 5 and board_sz <= 7 else 5
